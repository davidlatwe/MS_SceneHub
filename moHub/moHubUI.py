...
def mkShotListBtn(windowName, shotColumn):
	...
	for shot in shotList:
		...
		for num, asset in enumerate(shot_assetList):
			...
			geoRootPath = moGeoCache.getGeoCacheRoot()
			geoCacheDir = moGeoCache.getGeoCacheDir(geoRootPath, asset, 0, sceneName_GEO) if dateShortGEO else ''
...
def ui_geoCacheIO():
	...
	if True:
		...
		try:
			...
			projtext = text(l= 'Error', h= 36, al= 'left')
		setParent('..')
